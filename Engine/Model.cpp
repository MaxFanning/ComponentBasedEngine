...
bool Model::Load(const char* pFilename)
{
	FILE* pFile = nullptr;
	...
	for (u32 m = 0; m < numMeshes; ++m)
	{
		...
		meshBuffer->Initialize(Mesh::GetVertexFormat(), *mesh, numVertexWeights > 0);
...
void Model::Render(const Math::Matrix& transform) const
{
	MeshRenderer* renderer = MeshRenderer::Get();
