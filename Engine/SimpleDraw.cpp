...
void SimpleDrawImpl::Initialize(u32 maxVertices)
{
	...
	mVertexShader.Initialize(L"../Engine/Shaders/SimpleDraw.fx", "VS", "vs_4_0", kVertexFormat);
	mPixelShader.Initialize(L"../Engine/Shaders/SimpleDraw.fx", "PS", "ps_4_0");
	...
	GraphicsSystem::Get()->GetDevice()->CreateBuffer(&bd, nullptr, &mpVertexBuffer);
	GraphicsSystem::Get()->GetDevice()->CreateBuffer(&bd, nullptr, &mpVertexBuffer2D);
...
void SimpleDrawImpl::Render(const CameraComponent& camera)
{
	ASSERT(mInitialized, "[SimpleDraw] Not initialized.");
	...
	GraphicsSystem::Get()->GetContext()->Map(mpVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
	memcpy(resource.pData, mpVertices3D, mNumVertices3D * stride);
	GraphicsSystem::Get()->GetContext()->Unmap(mpVertexBuffer, 0);
	
	GraphicsSystem::Get()->GetContext()->IASetVertexBuffers(0, 1, &mpVertexBuffer, &stride, &offset);
	GraphicsSystem::Get()->GetContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
	GraphicsSystem::Get()->GetContext()->Draw(mNumVertices3D, 0);
	
	const u32 w = GraphicsSystem::Get()->GetWidth();
	const u32 h = GraphicsSystem::Get()->GetHeight();
	...
	// Draw 2D lines
	GraphicsSystem::Get()->GetContext()->Map(mpVertexBuffer2D, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
	memcpy(resource.pData, mpVertices2D, mNumVertices2D * stride);
	GraphicsSystem::Get()->GetContext()->Unmap(mpVertexBuffer2D, 0);
	
	GraphicsSystem::Get()->GetContext()->IASetVertexBuffers(0, 1, &mpVertexBuffer2D, &stride, &offset);
	GraphicsSystem::Get()->GetContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
	GraphicsSystem::Get()->GetContext()->Draw(mNumVertices2D, 0);
...
{
	AddAABB(aabb.center - aabb.extend, aabb.center + aabb.extend, color);
}
...
{
	AddAABB(center.x - extend, center.y - extend, center.z - extend, center.x + extend, center.y + extend, center.z + extend, color);
}
