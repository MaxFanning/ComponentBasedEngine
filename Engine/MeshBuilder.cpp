...
namespace
{
	...
	void FillSector(Sector& sector, u32 numColumns, u32 minX, u32 minZ, u32 maxX, u32 maxZ, const Mesh::Vertex* vertices)
	{
		sector.minX = minX;
		sector.minZ = minZ;
		sector.maxX = maxX;
		...
		for (u32 z = minZ; z <= maxZ; ++z)
		{
			...
			{
				const u32 vIndex = x + (z * numColumns);
				const Mesh::Vertex& vertex = vertices[vIndex];
				if (z == minZ && x == minX)
				{
					sector.aabb.center = vertex.position;
				}
				else
				{
					sector.aabb += vertex.position;
				}
			}
		}
	}

	void FillSectors(Sectors& sectors, u32 numColumns, u32 numRows, u32 sectorSize, const Mesh::Vertex* vertices)
	{
		u32 minZ = 0;
		u32 maxZ = sectorSize;
		for (; maxZ < numRows; minZ = maxZ, maxZ += sectorSize)
		{
			u32 minX = 0;
			u32 maxX = sectorSize;
			for (; maxX < numColumns; minX = maxX, maxX += sectorSize)
			{
				sectors.push_back(Sector());
				FillSector(sectors.back(), numColumns, minX, minZ, maxX, maxZ, vertices);
			}
...
void MeshBuilder::CreateTerrain(Mesh& mesh, Sectors& sectors, const Heightmap& heightmap, f32 maxHeight)
{
	...
	// Fill sectors
	FillSectors(sectors, kColumns, kRows, 10, vertices);
